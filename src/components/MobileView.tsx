...
const MobileMapComponent = dynamic<MobileMapProps>(() => import('./MobileMap'), { 
  ...
  loading: () => (
    <div style={{ width: '100%', height: '100%', background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '20px' }}>
      <div style={{ width: '32px', height: '32px', border: '3px solid #00ff9d', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
    </div>
  )
});
...
export default function MobileView({
  ...
  pharmacies.forEach(p => {
    if (!districtCounts[p.district]) {
      const colors = ['#00ff9d', '#14b8a6', '#6366f1', '#8b5cf6', '#ec4899'];
  ...
  // Theme colors
  const colors = {
    bg: isDarkMode ? '#050b14' : '#f0f4f8',
    bgGradient: isDarkMode ? 'linear-gradient(135deg, #0a1628 0%, #050b14 100%)' : 'linear-gradient(135deg, #f0f4f8 0%, #e2e8f0 100%)',
    card: isDarkMode ? 'rgba(15, 23, 42, 0.9)' : '#ffffff',
    cardHover: isDarkMode ? 'rgba(15, 23, 42, 0.95)' : '#f8fafc',
    cardBorder: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)',
    cardShadow: isDarkMode ? '0 4px 20px rgba(0,0,0,0.3)' : 'none',
    text: isDarkMode ? 'white' : '#1e293b',
    textSecondary: isDarkMode ? '#94a3b8' : '#475569',
    textMuted: isDarkMode ? '#64748b' : '#64748b',
    inputBg: isDarkMode ? 'rgba(255,255,255,0.05)' : '#ffffff',
    inputBorder: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.12)',
    buttonBg: isDarkMode ? 'rgba(255,255,255,0.05)' : '#f1f5f9',
    buttonBgHover: isDarkMode ? 'rgba(255,255,255,0.1)' : '#e2e8f0',
    iconBg: isDarkMode ? 'rgba(0,255,157,0.1)' : 'rgba(16,185,129,0.1)',
    accent: '#00ff9d',
    accentLight: isDarkMode ? '#00ff9d' : '#10b981',
  };
    ...
    <div style={{ height: '100vh', width: '100%', background: colors.bg, position: 'relative', overflow: 'hidden', fontFamily: "'Lexend', system-ui, sans-serif" }}>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <div style={{ 
              ...
              <svg width="36" height="36" viewBox="0 0 24 24" fill="#00ff9d">
      ...
      <div style={{ position: 'relative', zIndex: 10, height: '100%', display: 'flex', flexDirection: 'column', maxWidth: '440px', margin: '0 auto', padding: '20px' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', flexShrink: 0, position: 'relative', zIndex: 1000 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <img 
              ...
              style={{ width: '40px', height: '40px', borderRadius: '12px', boxShadow: isDarkMode ? '0 0 20px rgba(0,255,157,0.4)' : '0 4px 12px rgba(0,0,0,0.1)' }}
            />
            <div>
              <div style={{ fontSize: '18px', fontWeight: 700, color: colors.text }}>Nöbetçi<span style={{ color: '#00ff9d' }}> Eczane</span></div>
              <div style={{ fontSize: '10px', color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: '1px' }}>7/24 Hizmetinizde</div>
            </div>
          </div>
          <button 
            onClick={() => setShowMenu(!showMenu)}
            style={{ width: '40px', height: '40px', borderRadius: '50%', background: showMenu ? 'rgba(0,255,157,0.2)' : (isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)'), border: showMenu ? '1px solid rgba(0,255,157,0.3)' : `1px solid ${colors.cardBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backdropFilter: 'blur(10px)', transition: 'all 0.2s' }}
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill={showMenu ? '#00ff9d' : colors.text}><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
          </button>
          ...
          {showMenu && (
            <div style={{ 
              ...
              boxShadow: isDarkMode ? '0 20px 40px rgba(0,0,0,0.5)' : '0 10px 30px rgba(0,0,0,0.15)',
              minWidth: '200px',
              zIndex: 9999
            }}>
              <button onClick={() => handleMenuAction('nearby')} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'none', border: 'none', borderRadius: '12px', cursor: 'pointer', color: colors.text, fontSize: '14px', fontWeight: 500, textAlign: 'left' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="#00ff9d"><path d="M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm8.94 3c-.46-4.17-3.77-7.48-7.94-7.94V1h-2v2.06C6.83 3.52 3.52 6.83 3.06 11H1v2h2.06c.46 4.17 3.77 7.48 7.94 7.94V23h2v-2.06c4.17-.46 7.48-3.77 7.94-7.94H23v-2h-2.06zM12 19c-3.87 0-7-3.13-7-7s3.13-7 7-7 7 3.13 7 7-3.13 7-7 7z"/></svg>
              ...
              <button onClick={() => handleMenuAction('favorites')} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'none', border: 'none', borderRadius: '12px', cursor: 'pointer', color: colors.text, fontSize: '14px', fontWeight: 500, textAlign: 'left' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="#00ff9d"><path d="M17 3H7c-1.1 0-2 .9-2 2v16l7-3 7 3V5c0-1.1-.9-2-2-2z"/></svg>
            ...
            <>
              ...
              {!showPharmacyList ? (
                <div style={{ 
                  ...
                  padding: '28px', 
                  border: `1px solid ${colors.cardBorder}`,
                  boxShadow: isDarkMode ? '0 25px 50px rgba(0,0,0,0.5)' : '0 10px 40px rgba(0,0,0,0.1)',
                  marginBottom: '20px',
                  position: 'relative'
                }}>
                  <div style={{ position: 'absolute', top: 0, right: 0, width: '120px', height: '120px', background: 'rgba(0,255,157,0.05)', borderRadius: '50%', filter: 'blur(40px)', pointerEvents: 'none' }}></div>
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <div style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)' }}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="#00ff9d"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>
                      </div>
                      <select
                  ...
                  {selectedCountry !== 'Kuzey Kıbrıs' && (
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <div style={{ position: 'relative' }}>
                        <div style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)' }}>
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="#00ff9d"><path d="M15 11V5l-3-3-3 3v2H3v14h18V11h-6zm-8 8H5v-2h2v2zm0-4H5v-2h2v2zm0-4H5V9h2v2zm6 8h-2v-2h2v2zm0-4h-2v-2h2v2zm0-4h-2V9h2v2zm0-4h-2V5h2v2zm6 12h-2v-2h2v2zm0-4h-2v-2h2v2z"/></svg>
                        </div>
                        <select
                          value={selectedCity}
                  ...
                  <button
                    ...
                    style={{ width: '100%', background: selectedCity && selectedDistrict ? '#00ff9d' : '#334155', color: selectedCity && selectedDistrict ? '#050b14' : '#64748b', fontSize: '16px', fontWeight: 700, padding: '16px', borderRadius: '16px', border: 'none', cursor: selectedCity && selectedDistrict ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: selectedCity && selectedDistrict ? '0 0 30px rgba(0,255,157,0.4)' : 'none' }}
                  >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                    <button onClick={() => { setShowPharmacyList(false); setShowMiniMap(false); onReset(); }} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', color: '#00ff9d', fontSize: '14px', fontWeight: 600, cursor: 'pointer' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <button 
                        ...
                        style={{ 
                          ...
                          background: showMiniMap ? 'rgba(0,255,157,0.2)' : 'rgba(255,255,255,0.05)', 
                          border: showMiniMap ? '1px solid rgba(0,255,157,0.3)' : '1px solid rgba(255,255,255,0.1)', 
                          borderRadius: '8px', padding: '6px 10px', cursor: 'pointer', color: showMiniMap ? '#00ff9d' : '#64748b', fontSize: '11px', fontWeight: 600 
                        }}
                      >
                      ...
                      {[1, 2, 3, 4].map((i) => (
                        <div key={i} style={{ background: colors.card, borderRadius: '20px', padding: '20px', border: `1px solid ${colors.cardBorder}` }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                            ...
                            <div className="skeleton-pulse" style={{ width: '70px', height: '28px', borderRadius: '20px', background: isDarkMode ? 'rgba(0,255,157,0.1)' : 'rgba(16,185,129,0.1)' }} />
                          </div>
                          <div className="skeleton-pulse" style={{ width: '30%', height: '24px', borderRadius: '12px', background: isDarkMode ? 'rgba(0,255,157,0.15)' : 'rgba(16,185,129,0.1)', marginBottom: '16px' }} />
                          ...
                          <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                            <div className="skeleton-pulse" style={{ flex: 1, height: '44px', borderRadius: '12px', background: isDarkMode ? 'rgba(0,255,157,0.2)' : 'rgba(16,185,129,0.15)' }} />
                      ...
                      {sortedPharmacies.map(p => (
                        <div key={p.id} onClick={() => onSelectPharmacy(p)} style={{ 
                          ...
                          border: selectedPharmacy?.id === p.id 
                            ? `3px solid ${isDarkMode ? '#00ff9d' : '#10b981'}`
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: isDarkMode ? 'rgba(0,255,157,0.15)' : 'rgba(16,185,129,0.1)', padding: '6px 12px', borderRadius: '20px' }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill={isDarkMode ? '#00ff9d' : '#10b981'}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                              <span style={{ fontSize: '14px', fontWeight: 700, color: isDarkMode ? '#00ff9d' : '#10b981' }}>{formatDist(p.distance || 0)}</span>
                            </div>
                          </div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: isDarkMode ? 'rgba(255,255,255,0.03)' : '#f9fafb', borderRadius: '12px' }}>
                              <div style={{ width: '32px', height: '32px', background: isDarkMode ? 'rgba(0,255,157,0.1)' : 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: isDarkMode ? 'none' : '1px solid #e5e7eb' }}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill={isDarkMode ? '#00ff9d' : '#10b981'}><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                              </div>
                              <p style={{ flex: 1, fontSize: '13px', fontWeight: 500, color: colors.text, margin: 0, lineHeight: 1.4 }}>{p.address}</p>
                              <button onClick={e => { e.stopPropagation(); copyToClipboard(p.address, `addr-${p.id}`); }} style={{ background: copiedText === `addr-${p.id}` ? (isDarkMode ? 'rgba(0,255,157,0.2)' : 'rgba(16,185,129,0.15)') : (isDarkMode ? 'rgba(255,255,255,0.1)' : '#e5e7eb'), border: 'none', borderRadius: '8px', padding: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: copiedText === `addr-${p.id}` ? (isDarkMode ? '#00ff9d' : '#10b981') : colors.textMuted, transition: 'all 0.2s' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px', background: isDarkMode ? 'rgba(255,255,255,0.03)' : '#f9fafb', borderRadius: '12px' }}>
                              <div style={{ width: '32px', height: '32px', background: isDarkMode ? 'rgba(0,255,157,0.1)' : 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: isDarkMode ? 'none' : '1px solid #e5e7eb' }}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill={isDarkMode ? '#00ff9d' : '#10b981'}><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>
                              </div>
                              <p style={{ flex: 1, fontSize: '14px', fontWeight: 600, color: colors.text, margin: 0 }}>{p.phone}</p>
                              <button onClick={e => { e.stopPropagation(); copyToClipboard(p.phone, `phone-${p.id}`); }} style={{ background: copiedText === `phone-${p.id}` ? (isDarkMode ? 'rgba(0,255,157,0.2)' : 'rgba(16,185,129,0.15)') : (isDarkMode ? 'rgba(255,255,255,0.1)' : '#e5e7eb'), border: 'none', borderRadius: '8px', padding: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: copiedText === `phone-${p.id}` ? (isDarkMode ? '#00ff9d' : '#10b981') : colors.textMuted, transition: 'all 0.2s' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                            <button onClick={e => { e.stopPropagation(); call(p.phone); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: isDarkMode ? '#00ff9d' : '#10b981', border: 'none', borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: 600, color: isDarkMode ? 'black' : 'white', cursor: 'pointer' }}>
                            ...
                            <button onClick={e => { e.stopPropagation(); savePharmacy(p); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: isPharmacySaved(p.id) ? (isDarkMode ? 'rgba(0,255,157,0.15)' : 'rgba(16,185,129,0.1)') : 'transparent', border: `1.5px solid ${isPharmacySaved(p.id) ? (isDarkMode ? '#00ff9d' : '#10b981') : (isDarkMode ? 'rgba(255,255,255,0.15)' : '#d1d5db')}`, borderRadius: '12px', padding: '14px', fontSize: '14px', fontWeight: 600, color: isPharmacySaved(p.id) ? (isDarkMode ? '#00ff9d' : '#10b981') : colors.textSecondary, cursor: 'pointer' }}>
                    ...
                    return (
                      <div 
                        ...
                        style={{ 
                          flexShrink: 0, 
                          width: '280px', 
                          background: isSelected ? (isDarkMode ? 'rgba(0,255,157,0.15)' : 'rgba(16,185,129,0.1)') : colors.card, 
                          backdropFilter: 'blur(24px)', 
                          borderRadius: '16px', 
                          padding: '14px', 
                          cursor: 'pointer', 
                          border: isSelected ? `2px solid ${isDarkMode ? '#00ff9d' : '#10b981'}` : `1px solid ${colors.cardBorder}`,
                          boxShadow: isSelected ? `0 0 20px ${isDarkMode ? 'rgba(0,255,157,0.3)' : 'rgba(16,185,129,0.2)'}` : colors.cardShadow,
                          scrollSnapAlign: 'start',
                          transition: 'all 0.2s ease'
                        }}
                      >
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '10px' }}>
                          <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: isDarkMode ? 'linear-gradient(135deg, #0a1628, #050b14)' : 'linear-gradient(135deg, #ffffff, #f1f5f9)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: isDarkMode ? '0 0 8px rgba(0,255,157,0.2)' : '0 2px 6px rgba(0,0,0,0.1)', border: isDarkMode ? '1px solid rgba(0,255,157,0.15)' : '1px solid #e2e8f0' }}>
                            <svg width="20" height="20" viewBox="0 0 24 24">
                              <rect x="10" y="4" width="4" height="16" rx="1" fill={isDarkMode ? '#00ff9d' : '#10b981'}/>
                              <rect x="4" y="10" width="16" height="4" rx="1" fill={isDarkMode ? '#00ff9d' : '#10b981'}/>
                              <circle cx="12" cy="12" r="2" fill={isDarkMode ? '#050b14' : '#ffffff'}/>
                              <circle cx="12" cy="12" r="1" fill={isDarkMode ? '#00ff9d' : '#10b981'}/>
                            </svg>
                          </div>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                              ...
                              <span style={{ fontSize: '12px', fontWeight: 700, color: isDarkMode ? '#00ff9d' : '#059669', flexShrink: 0 }}>{formatDist(p.distance || 0)}</span>
                            </div>
                        ...
                        {isSelected && (
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px' }}>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); directions(p.lat, p.lng); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', background: isDarkMode ? '#00ff9d' : '#10b981', border: 'none', borderRadius: '8px', padding: '8px', fontSize: '11px', fontWeight: 700, color: isDarkMode ? 'black' : 'white', cursor: 'pointer' }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71z"/></svg>
                              Git
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); savePharmacy(p); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', background: isPharmacySaved(p.id) ? (isDarkMode ? 'rgba(0,255,157,0.2)' : 'rgba(16,185,129,0.15)') : colors.buttonBg, border: isPharmacySaved(p.id) ? '1px solid rgba(0,255,157,0.3)' : `1px solid ${colors.cardBorder}`, borderRadius: '8px', padding: '8px', fontSize: '11px', fontWeight: 600, color: isPharmacySaved(p.id) ? colors.accentLight : colors.text, cursor: 'pointer' }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '10px' }}>
                          ...
                          <button onClick={() => directions(item.lat, item.lng)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', background: isDarkMode ? '#00ff9d' : '#10b981', border: 'none', borderRadius: '10px', padding: '10px', fontSize: '12px', fontWeight: 700, color: isDarkMode ? 'black' : 'white', cursor: 'pointer' }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71z"/></svg>
                            Git
                          </button>
            ...
            <div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button 
                    ...
                    style={{ 
                      ...
                      padding: '10px',
                      borderRadius: '10px',
                      border: themeMode === 'auto' ? '2px solid #00ff9d' : `1px solid ${colors.cardBorder}`,
                      background: themeMode === 'auto' ? 'rgba(0,255,157,0.1)' : colors.card,
                      cursor: 'pointer',
                      display: 'flex',
                      flexDirection: 'column',
                      alignItems: 'center',
                  ...
                  <button 
                    ...
                    style={{ 
                      ...
                      padding: '10px',
                      borderRadius: '10px',
                      border: themeMode === 'dark' ? '2px solid #00ff9d' : `1px solid ${colors.cardBorder}`,
                      background: themeMode === 'dark' ? 'rgba(0,255,157,0.1)' : colors.card,
                      cursor: 'pointer',
                      display: 'flex',
                      flexDirection: 'column',
                      alignItems: 'center',
                  ...
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill={themeMode === 'dark' ? '#00ff9d' : colors.textSecondary}><path d="M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9 9-4.03 9-9c0-.46-.04-.92-.1-1.36-.98 1.37-2.58 2.26-4.4 2.26-2.98 0-5.4-2.42-5.4-5.4 0-1.81.89-3.42 2.26-4.4-.44-.06-.9-.1-1.36-.1z"/></svg>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: themeMode === 'dark' ? '#00ff9d' : colors.textSecondary }}>Karanlık</span>
                  </button>
                  <button 
                    onClick={onToggleTheme}
                    style={{ 
                      ...
                      padding: '10px',
                      borderRadius: '10px',
                      border: themeMode === 'light' ? '2px solid #00ff9d' : `1px solid ${colors.cardBorder}`,
                      background: themeMode === 'light' ? 'rgba(0,255,157,0.1)' : colors.card,
                      cursor: 'pointer',
                      display: 'flex',
                      flexDirection: 'column',
                      alignItems: 'center',
                  ...
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill={themeMode === 'light' ? '#00ff9d' : colors.textSecondary}><path d="M12 7c-2.76 0-5 2.24-5 5s2.24 5 5 5 5-2.24 5-5-2.24-5-5-5zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm18 0h2c.55 0 1-.45 1-1s-.45-1-1-1h-2c-.55 0-1 .45-1 1s.45 1 1 1zM11 2v2c0 .55.45 1 1 1s1-.45 1-1V2c0-.55-.45-1-1-1s-1 .45-1 1zm0 18v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1s-1 .45-1 1zM5.99 4.58c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0s.39-1.03 0-1.41L5.99 4.58zm12.37 12.37c-.39-.39-1.03-.39-1.41 0-.39.39-.39 1.03 0 1.41l1.06 1.06c.39.39 1.03.39 1.41 0 .39-.39.39-1.03 0-1.41l-1.06-1.06zm1.06-10.96c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06zM7.05 18.36c.39-.39.39-1.03 0-1.41-.39-.39-1.03-.39-1.41 0l-1.06 1.06c-.39.39-.39 1.03 0 1.41s1.03.39 1.41 0l1.06-1.06z"/></svg>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: themeMode === 'light' ? '#00ff9d' : colors.textSecondary }}>Açık</span>
                  </button>
                </div>
              </div>
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '16px', border: `1px solid ${colors.cardBorder}`, marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ width: '44px', height: '24px', borderRadius: '12px', background: userLocation ? '#00ff9d' : '#334155', position: 'relative' }}>
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '16px', border: `1px solid ${colors.cardBorder}`, marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '14px', color: '#00ff9d' }}>v1.0.0</span>
                </div>
              </div>
              <div style={{ background: colors.card, borderRadius: '16px', padding: '16px', border: `1px solid ${colors.cardBorder}`, marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '14px', color: '#00ff9d' }}>{savedPharmacies.length}</span>
                </div>
              </div>
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '16px', border: `1px solid ${colors.cardBorder}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: isDarkMode ? 'rgba(0,255,157,0.1)' : 'rgba(16,185,129,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill={isDarkMode ? '#00ff9d' : '#10b981'}><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 10.99h7c-.53 4.12-3.28 7.79-7 8.94V12H5V6.3l7-3.11v8.8z"/></svg>
                  </div>
                  <div>
        ...
        <nav style={{ 
          ...
          <button onClick={() => { setActiveTab('search'); setShowMenu(false); }} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'search' ? (isDarkMode ? '#00ff9d' : '#059669') : (isDarkMode ? '#64748b' : '#64748b'), padding: '4px 0' }}>
          ...
          <button onClick={() => { setActiveTab('map'); setShowMenu(false); }} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'map' ? (isDarkMode ? '#00ff9d' : '#059669') : (isDarkMode ? '#64748b' : '#64748b'), padding: '4px 0' }}>
          ...
          <button 
            ...
            style={{ 
              ...
              width: '56px', 
              height: '56px', 
              background: nearestPharmacy ? '#00ff9d' : (isDarkMode ? '#334155' : '#e2e8f0'), 
              ...
              boxShadow: nearestPharmacy ? '0 0 30px rgba(0,255,157,0.4)' : (isDarkMode ? '0 10px 30px rgba(0,0,0,0.5)' : '0 10px 30px rgba(0,0,0,0.15)'),
              transition: 'all 0.2s ease'
            }}
          >
          ...
          <button onClick={() => { setActiveTab('saved'); setShowMenu(false); }} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'saved' ? (isDarkMode ? '#00ff9d' : '#059669') : (isDarkMode ? '#64748b' : '#64748b'), padding: '4px 0' }}>
          ...
          <button onClick={() => { setActiveTab('settings'); setShowMenu(false); }} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: activeTab === 'settings' ? (isDarkMode ? '#00ff9d' : '#059669') : (isDarkMode ? '#64748b' : '#64748b'), padding: '4px 0' }}>
      ...
      <style>{`
        ...
        *::-webkit-scrollbar-thumb { background: rgba(0,255,157,0.3); border-radius: 10px; }
        *::-webkit-scrollbar-thumb:hover { background: rgba(0,255,157,0.5); }
        * { scrollbar-width: thin; scrollbar-color: rgba(0,255,157,0.3) transparent; }
      `}</style>
    </div>
  );
}
